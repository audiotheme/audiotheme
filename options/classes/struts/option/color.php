...
class Struts_Option_Color extends Struts_Option {
	...
	public function input_html() {
		...
		echo '<div id="' . esc_attr( $id . '-color-chooser' ) . '" class="struts-color-chooser" data-field-id="' . esc_attr( $id ) . '"></div>';
		echo '<input type="text" id="' . esc_attr( $id ) . '" name="' . esc_attr( $name ) . '" value="' . esc_attr( $value ) . '" class="struts-color-chooser-input" />';
	}
	...
	protected function label_html() {
		if ( $this->label() ) {
			echo '<label class="struts-label" for="' . esc_attr( $this->html_id() ) . '">' . esc_html( $this->label() ) . ' ';
			echo '<a href="#" class="struts-color-chooser-toggle">' . __( 'show color picker', 'audiotheme' ) . '</a>';
			echo "</label>";
		}
	}
