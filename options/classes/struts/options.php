...
class Struts_Options {
	...
	public function add_option( $name, $type, $section = NULL ) {
		...
		if ( NULL !== $section ) {
			...
			if ( ! isset( $sections[$section] ) ) {
				throw new SectionNotFoundException( sprintf( __( 'Section with name &lsquo;%1$s&rsquo; not defined', 'audiotheme' ), esc_html( $section ) ) );
	...
	public function echo_form_html() { ?>
		<div id="struts-options" class="wrap">
			...
			<h2><?php _e( 'Theme Options', 'audiotheme' ); ?></h2>
			<div id="struts-options" class="wrap">
				<div id="struts-options-body">
					...
					<form action="options.php" method="post">
						...
						<div class="struts-buttons-container">
							<input name="<?php echo esc_attr( $this->name() ); ?>[struts_submit]" type="submit" class="button-primary struts-save-button" value="<?php esc_attr_e( 'Save Settings', 'audiotheme' ); ?>" />
							<input name="<?php echo esc_attr( $this->name() ); ?>[struts_reset]" type="submit" class="button-secondary struts-reset-button" value="<?php esc_attr_e( 'Reset Defaults', 'audiotheme' ); ?>" onclick="return confirm('<?php esc_attr_e( 'Click OK if you want to reset your theme options to the defaults.', 'audiotheme' ) ?>')" />
						</div>
					</form>
				</div>
			</div>
	...
	public function settings_updated_html() {
		if ( isset( $_GET['settings-updated'] ) )
			return "<div class='updated'><p>" . __( 'Theme settings updated successfully.', 'audiotheme' ) . "</p></div>";
	}
