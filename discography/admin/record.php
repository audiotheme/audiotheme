...
function audiotheme_record_list_help() {
	...
	get_current_screen()->add_help_tab( array(
		...
		'content' =>
			...
			'<ul>' .
			'<li>' . __( "You can hide or display columns based on your needs and decide how many records to list per screen using the Screen Options tab.", 'audiotheme-i18n' ) . '</li>' .
			'<li>' . __( "You can filter the list of records by status using the text links in the upper left to show All, Published, Draft, or Trashed records. The default view is to show all records.", 'audiotheme-i18n' ) . '</li>' .
	...
	get_current_screen()->add_help_tab( array(
		...
		'content' =>
			'<p>' . __( "Hovering over a row in the records list will display action links that allow you to manage your record. You can perform the following actions:", 'audiotheme-i18n' ) . '</p>' .
			'<ul>' .
			'<li>' . __( "<strong>Edit</strong> takes you to the editing screen for that record. You can also reach that screen by clicking on the record name.", 'audiotheme-i18n' ) . '</li>' .
			'<li>' . __( "<strong>Trash</strong> removes your record from this list and places it in the trash, from which you can permanently delete it.", 'audiotheme-i18n' ) . '</li>' .
			'<li>' . __( "<strong>Preview</strong> will show you what your draft record will look like if you publish it.", 'audiotheme-i18n' ) . '</li>' .
			'<li>' . __( "<strong>View</strong> will take you to your live site to view the record. Which link is available depends on your record's status.", 'audiotheme-i18n' ) . '</li>' .
			'</ul>',
	) );
}
...
function audiotheme_record_help() {
	...
	get_current_screen()->add_help_tab( array(
		...
		'content' =>
			'<p>' . __( "<strong>Title</strong> - This is the name of your record. If your record is an album, put the album title here. If it's a single, enter the single's title. If you'd rather add all of your singles as a collection, then you'll need to give that collection a name here.", 'audiotheme-i18n' ) . '</p>' .
			...
			'<h4>' . __( 'Track List', 'audiotheme-i18n' ) . '</h4>' .
			'<p>' . __( "Tracks are the individual songs that make up your record. Use the Add Track button to quickly add as many as you'd like.", 'audiotheme-i18n' ) . '</p>' .
			'<ul> ' .
	...
	get_current_screen()->add_help_tab( array(
		...
		'content' 	=>
			...
			'<ul>' .
			'<li>' . __( "<strong>Release Year</strong> - Help fans differentiate between new and less new recordings.", 'audiotheme-i18n' ) . '</li>' .
			'<li>' . __( "<strong>Artist</strong> - Use this field for the main artist of the record.", 'audiotheme-i18n' ) . '</li>' .
			'<li>' . __( "<strong>Genre</strong> - What genre would you classify your record as?", 'audiotheme-i18n' ) . '</li>' .
			'<li>' . __( "<strong>Type</strong> - Does your record consist of an album or single(s)?", 'audiotheme-i18n' ) . '</li>' .
