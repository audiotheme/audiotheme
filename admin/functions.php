...
function audiotheme_post_submit_meta_box( $post, $metabox ) {
	...
	<div class="submitbox" id="submitpost">
		...
		<div id="minor-publishing">
			...
			<div style="display: none"><?php submit_button( __( 'Save' ), 'button', 'save' ); ?></div>
				...
				if ( false !== $show_statuses ) :
					...
					<div class="misc-pub-section<?php if ( ! $can_publish || ( ! $show_visibility && ! $show_publish_date ) ) { echo ' misc-pub-section-last'; } ?>">
						...
						<?php if ( 'publish' == $post->post_status || 'private' == $post->post_status || ( $can_publish && count( $show_statuses ) ) ) { ?>
							<a href="#post_status" class="edit-post-status hide-if-no-js" tabindex="4" <?php if ( 'private' == $post->post_status ) { echo 'style="display: none"'; } ?>><?php _e('Edit') ?></a>
							
							<div id="post-status-select" class="hide-if-js">
								<input type="hidden" name="hidden_post_status" id="hidden_post_status" value="<?php echo esc_attr( ( 'auto-draft' == $post->post_status ) ? 'draft' : $post->post_status ); ?>">
								<select name="post_status" id="post_status" tabindex="4">
									<?php if ( 'publish' == $post->post_status ) : ?>
										<option value="publish" <?php selected( $post->post_status, 'publish' ); ?>><?php _e( 'Published' ) ?></option>
									<?php elseif ( 'private' == $post->post_status ) : ?>
										<option value="publish" <?php selected( $post->post_status, 'private' ); ?>><?php _e( 'Privately Published' ) ?></option>
									<?php elseif ( 'future' == $post->post_status ) : ?>
										<option value="future" <?php selected( $post->post_status, 'future' ); ?>><?php _e( 'Scheduled' ) ?></option>
									<?php endif; ?>
									
									<?php if ( array_key_exists( 'pending', $show_statuses ) ) : ?>
										<option value="pending" <?php selected( $post->post_status, 'pending' ); ?>><?php _e( 'Pending Review' ) ?></option>
									<?php endif; ?>
									
									<?php if ( 'auto-draft' == $post->post_status ) : ?>
										<option value="draft" <?php selected( $post->post_status, 'auto-draft' ); ?>><?php _e( 'Draft' ) ?></option>
									<?php else : ?>
										<option value="draft" <?php selected( $post->post_status, 'draft' ); ?>><?php _e( 'Draft' ) ?></option>
									<?php endif; ?>
								</select>
					...
					<div class="misc-pub-section" id="visibility">
						...
						<?php if ( $can_publish ) { ?>
							...
							<div id="post-visibility-select" class="hide-if-js">
								...
								<label for="visibility-radio-public" class="selectit"><?php _e('Public'); ?></label>
								<br>
								...
								<?php if ( 'post' == $post_type && current_user_can( 'edit_others_posts' ) ) : ?>
									<span id="sticky-span">
										...
										<label for="sticky" class="selectit"><?php _e( 'Stick this post to the front page' ); ?></label>
										<br>
									</span>
								<?php endif; ?>
								...
								<label for="visibility-radio-private" class="selectit"><?php _e('Private'); ?></label>
								<br>
			...
			<?php if ( 'auto-draft' != $post->post_status ) : ?>
				<div id="delete-action">
					...
					if ( current_user_can( 'delete_post', $post->ID ) ) {
						...
						?>
						<a class="submitdelete deletion" href="<?php echo get_delete_post_link( $post->ID , '', $force_delete ); ?>"<?php echo $onclick; ?>><?php echo $delete_text; ?></a>
					<?php } ?>
				</div>
			<?php endif; ?>
				...
				if ( ! in_array( $post->post_status, array( 'publish', 'future', 'private' ) ) || 0 == $post->ID ) {
					if ( $can_publish ) :
						if ( ! empty( $post->post_date_gmt ) && time() < strtotime( $post->post_date_gmt . ' +0000' ) ) : ?>
							<input type="hidden" name="original_publish" id="original_publish" value="<?php esc_attr_e( 'Schedule' ); ?>">
						...
						<?php endif; ?>
					<?php else : ?>
						<input type="hidden" name="original_publish" id="original_publish" value="<?php esc_attr_e( 'Submit for Review' ) ?>">
						<?php
						submit_button( __( 'Submit for Review' ), 'primary', 'publish', false, array( 'tabindex' => '5', 'accesskey' => 'p' ) );
					endif;
				} else { ?>
