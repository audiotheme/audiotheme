...
class AudioTheme_Module_Videos_Admin {
	/**
	...
	public function load() {
		add_action( 'save_post', 'on_save_video', 10, 2 );
		add_action( 'wp_ajax_audiotheme_get_video_oembed_data', 'ajax_get_video_oembed_data' );
	
		add_filter( 'post_updated_messages', 'video_updated_messages' );
		add_filter( 'manage_edit-audiotheme_video_columns', 'register_columns' );
		add_action( 'add_meta_boxes_audiotheme_video', 'register_meta_boxes' );
		add_filter( 'admin_post_thumbnail_html', 'video_post_thumbnail_html', 10, 2 );
	 ...
	 * @since 1.0.0
	 */
	function display_video_url_field() {
		global $post;
	 ...
	 * @return string
	 */
	function video_post_thumbnail_html( $content, $post_id ) {
	 ...
	 * @since 1.0.0
	 */
	function ajax_get_video_oembed_data() {
		global $post_id;
	 ...
	 * @return string
	 */
	function parse_video_oembed_data( $return, $data, $url ) {
		global $post_id;
	 ...
	 * @since 1.0.0
	 */
	function add_video_thumbnail( $attachment_id ) {
		global $post_id;
	 ...
	 */
	function on_save_video( $post_id, $post ) {
	 ...
	 */
	function on_save_archive_settings( $post_id, $post, $post_type ) {
	 ...
	 * @param WP_Post $post Post object.
	 */
	function archive_settings( $post ) {
